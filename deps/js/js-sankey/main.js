...
function drawSankey(graph, height_g, width_g) {
  $('#chart').attr('height',height_g);
var margin = {
        top: 20,
        right: 1,
        bottom: 30,
    ...
    a.forEach(function (x) {svg.selectAll(x).classed("selected", true)});
    }

//    d3.data(data, function(energy) {
      ...
      var link = svg.append("g").selectAll(".link")
        .data(graph.links)
        .enter().append("path")
    .attr("class", function (d) {return "link trace_" + d.id})
    .on("mouseover", function(d) {
      ...
      link.append("title")
        .text(function(d) {
          return  format(d.value) + " " + d.source.name + " → " + d.target.name;
        });
    ...
    if ((x.source == d) || (x.target == d)) s.add(".trace_" + x.id);
    })
    select(s);
         })
        .append("title")
        .text(function(d) {
          return format(d.value) + " " + d.legende;
        });

     var texte = node.append("text")
    .attr("class","code")
        .attr("x", -6)
        .attr("y", function(d) {
          return d.dy / 2;
